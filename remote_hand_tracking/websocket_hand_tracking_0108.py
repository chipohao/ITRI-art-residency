...
class PoseDetector:
    ...
    # === 平滑化改善修正（位置 EMA） START ===
    def smooth_point(self, new_point):
        """
        使用 Exponential Moving Average (EMA) 平滑座標
        抑制 MediaPipe landmark frame-to-frame 抖動
        """
    ...
    # =========================================================
    # === 平滑化改善修正（速度：瞬時速度 + EMA，低延遲）===
    # =========================================================
    def calculate_velocity(self):
        """
        使用「瞬時速度 + EMA」降低延遲
        位置已平滑，速度 EMA 係數刻意設較大以提升即時性
        """
        ...
        instant_velocity = math.hypot(x2 - x1, y2 - y1) / dt

        # === 平滑化改善修正：速度 EMA ===
        if self.smoothed_velocity is None:
            self.smoothed_velocity = instant_velocity
        else:
            velocity_alpha = 0.4  # ⭐ 比位置 EMA 快，降低 lag
            self.smoothed_velocity += velocity_alpha * (
                instant_velocity - self.smoothed_velocity
            )

        return round(self.smoothed_velocity, 2)
